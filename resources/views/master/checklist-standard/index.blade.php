...
@section('buttons')
    @if ((int)$privileges["create"] == 1)
        <a href="{{ url('master/checklist-standards/create')}}" class="btn btn-sm btn-success"> Add Checklist Standard</a>
    @endif
@endsection
@section('content')
                    ...
                    @forelse($checklistStandards as $checklistStandard)
                        <tr>
                            ...
                            <td width="18%" class="text-center">
                                <a href="{{ url('master/checklist-standards/' . $checklistStandard->id) }}" class="btn btn-outline-primary btn-sm" title="Detail"><i class="fas fa-list"></i></a>
                                @if ((int)$privileges->edit == 1)
                                <a href="{{ url('master/checklist-standards/' . $checklistStandard->id . '/edit') }}" class="btn btn-sm btn-outline-info" title="Edit"><i class="fas fa-edit"></i></a>
                                @endif
                                @if((int)$privileges->delete == 1)
                                <a href="#" class="form-confirm  btn btn-sm btn btn-outline-danger" title="Delete">
